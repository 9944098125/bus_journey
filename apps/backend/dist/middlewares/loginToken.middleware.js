 ...
 */
export const validateFirstLoginToken = async (req, res, next) => {
    try {
        ...
        if (user.is_verified) {
            res.status(400).json({
                success: false,
                message: "Account is already activated. Please sign in with your email or phone and password.",
            });
            return;
        }
 ...
 */
export const requireVerifiedForPasswordLogin = async (req, res, next) => {
    try {
        ...
        const user = await Users.findOne(email?.trim()
            ? { email: email.trim().toLowerCase() }
            : { phone_number: phone_number?.trim() }).select("is_verified email");
        if (user && !user.is_verified) {
            res.status(403).json({
                success: false,
                message: "Please activate your account using the login link sent after registration.",
