...
const portsToTry = [
	...new Set(
		[Number(process.env.PORT), ...FALLBACK_PORTS].filter(
			(port): port is number => Number.isFinite(port) && port > 0,
		),
	),
];

console.log(
	"[DEBUG] Boot — PORT from env:",
	process.env.PORT,
	"→ will try:",
	portsToTry.join(", "),
);
...
const startServer = async (): Promise<void> => {
	try {
		...
		if (!server || activePort === undefined) {
			console.error(
				`❌ No available port. Tried: ${portsToTry.join(", ")}. Stop other processes (e.g. pnpm dev) or set PORT in .env`,
			);
