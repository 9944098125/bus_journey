...
import { RECENT_USERS } from '../utils/dashboard-mock-data';
import { AUTH_PROVIDER_LABELS, formatRelativeTime } from '../utils/dashboard-utils';
    ...
    <article className="admin-card-surface overflow-hidden rounded-2xl border">
      <header className="border-b border-sea-light/40 px-5 py-4 sm:px-6">
        ...
        <p className="mt-0.5 text-[1.2rem] text-sea-mid/70">
          Sample rows with full_name, contact, role, auth_provider, flags &amp;
          last_login_at
        </p>
      </header>
              ...
              return (
                <tr
                  ...
                  className={cn(
                    ...
                    index === RECENT_USERS.length - 1 && 'border-b-0',
                  )}
                >
                  ...
                  <td className="px-3 py-3.5">
                    <span className="rounded-md bg-sea-pale/50 px-2 py-0.5 text-[1.1rem] font-medium text-sea-deep">
                      {user.auth_provider}
                    </span>
                    <p className="mt-0.5 text-[1rem] text-sea-mid/60">
